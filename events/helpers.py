#From https://github.com/palmtree5/palmtree5-cogs/blob/redv3-rewrites/eventmaker/helpers.py
# Thank you palmtree5 https://github.com/palmtree5
...
def get_event_embed(guild: discord.Guild, now: dt, event: dict) -> discord.Embed:
    ...
    created_delta_str = get_delta_str(dt.fromtimestamp(event["create_time"]), now)
    created_str = "{} ago (at {} UTC)".format(
        created_delta_str, dt.fromtimestamp(event["create_time"]).strftime("%Y-%m-%d %H:%M:%S")
    )

    start_delta_str = get_delta_str(now, dt.fromtimestamp(event["event_start_time"]))
    if event["has_started"]:
        start_str = "Already started!"
    else:
        start_str = "In {} (at {} UTC)".format(
            start_delta_str,
            dt.fromtimestamp(event["event_start_time"]).strftime("%Y-%m-%d %H:%M:%S"),
        )
